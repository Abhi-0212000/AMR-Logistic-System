...
from .error_handling import handle_error, create_error_response, ErrorType
...
class CentralManagementNode(Node):
    def __init__(self):
        super().__init__('central_management_node')
    ...
    async def _handle_goal_request(self, request: AmrGoalManagement.Request, 
                                 response: AmrGoalManagement.Response) -> AmrGoalManagement.Response:
        """Handle incoming goal requests with proper error handling and state management."""
        """Handle incoming goal requests."""
        if self._robot_state != RobotState.IDLE:
            return create_error_response( ErrorType.BUSY_ERROR,"Another goal is currently being processed")
        ...
        try:
            ...
            if path_response.status != 0:
                self._robot_state = RobotState.IDLE
                return create_error_response(ErrorType.PLANNING_ERROR, f"Path planning failed: {path_response.message}")
            ...
            if not goal_accepted:
                self._robot_state = RobotState.IDLE
                return create_error_response(ErrorType.GOAL_REJECTED, "Goal rejected by action server")
            ...
            self._robot_state = RobotState.ERROR
            return create_error_response(response, f"Unexpected error: {str(e)}", 
                                      ErrorType.SYSTEM_ERROR)

    async def _request_path_plan(self, request: AmrGoalManagement.Request) -> ComputeGlobalPath.Response:
    ...
    async def _send_navigation_goal(self, path_response: ComputeGlobalPath.Response, 
                                  original_request: AmrGoalManagement.Request) -> bool:
        ...
        try:
            ...
            self.get_logger().info('Add done callback is called aysnchronously and Waiting for navigation result...')
