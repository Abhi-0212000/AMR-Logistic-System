...
def create_error_response(
    error_type: ErrorType, error_msg: Optional[str] = None, status_code: int = 1
) -> AmrGoalManagement.Response:
    """
    Create and returns a standardized error response.
...
def check_response_status(response: AmrGoalManagement.Response) -> bool:
    """
    Check if a response indicates success or failure.
...
def log_error_with_context(
    node: Node, error: Exception, context: str, error_type: ErrorType
) -> None:
    """
...
def create_service_response(
    success: bool, message: str, status_code: int = None
) -> AmrGoalManagement.Response:
    """
    Create a service response with standard format.
