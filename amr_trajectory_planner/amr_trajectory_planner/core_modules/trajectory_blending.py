...
from amr_trajectory_planner.core_modules.spline_generation import BezierSplineGenerator
...
class TrajectoryBlender:
    ...
    def find_join_waypoint(self, points: List[PlanningPoint], join_time: float) -> Optional[int]:
        """
    ...
    def extract_blend_data(
        self, current_trajectory: TrajectoryInfo, join_point_idx: int
    ) -> BlendingConstraints:
        """
        ...
        first_deriv = BezierSplineGenerator.calculate_derivative_from_control_points(
            segment.control_points, param_t, order=1
        )
        second_deriv = BezierSplineGenerator.calculate_derivative_from_control_points(
            segment.control_points, param_t, order=2
        )
    ...
    def calculate_next_window_time(
        self, planning_time: float, total_time: float, buffer_time: float = 0.5
    ) -> float:
        """
