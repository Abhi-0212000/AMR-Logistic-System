    ...
    class ParamListener:
        ...
        def declare_params(self):
            ...
            if not self.node_.has_parameter(self.prefix_ + "robot_constraints.v_max"):
                descriptor = ParameterDescriptor(
                    description="Maximum translational velocity (m/s)", read_only=True
                )
                descriptor.floating_point_range.append(FloatingPointRange())
                descriptor.floating_point_range[-1].from_value = 0.1
                ...
                self.node_.declare_parameter(
                    self.prefix_ + "robot_constraints.v_max", parameter, descriptor
                )

            if not self.node_.has_parameter(self.prefix_ + "robot_constraints.omega_max"):
                descriptor = ParameterDescriptor(
                    description="Maximum rotational velocity (rad/s)", read_only=True
                )
                descriptor.floating_point_range.append(FloatingPointRange())
                descriptor.floating_point_range[-1].from_value = 0.1
                descriptor.floating_point_range[-1].to_value = 2.0
                ...
                self.node_.declare_parameter(
                    self.prefix_ + "robot_constraints.omega_max", parameter, descriptor
                )

            if not self.node_.has_parameter(self.prefix_ + "robot_constraints.a_t_max"):
                descriptor = ParameterDescriptor(
                    description="Maximum translational acceleration/deceleration (m/s²)",
                    read_only=True,
                )
                descriptor.floating_point_range.append(FloatingPointRange())
                descriptor.floating_point_range[-1].from_value = 0.1
                ...
                self.node_.declare_parameter(
                    self.prefix_ + "robot_constraints.a_t_max", parameter, descriptor
                )

            if not self.node_.has_parameter(self.prefix_ + "robot_constraints.a_r_max"):
                descriptor = ParameterDescriptor(
                    description="Maximum rotational acceleration/deceleration (rad/s²)",
                    read_only=True,
                )
                descriptor.floating_point_range.append(FloatingPointRange())
                descriptor.floating_point_range[-1].from_value = 0.1
                ...
                self.node_.declare_parameter(
                    self.prefix_ + "robot_constraints.a_r_max", parameter, descriptor
                )

            if not self.node_.has_parameter(self.prefix_ + "robot_constraints.f_max"):
                descriptor = ParameterDescriptor(
                    description="Maximum centripetal force (N) 40N (for a robot with mass m = 10 kg and min turning radius r = 1 m. i.e f = m * v^2 / r and v = 2.0 m/s i.e max velocity)",
                    read_only=True,
                )
                descriptor.floating_point_range.append(FloatingPointRange())
                descriptor.floating_point_range[-1].from_value = 0.1
                ...
                self.node_.declare_parameter(
                    self.prefix_ + "robot_constraints.f_max", parameter, descriptor
                )

            if not self.node_.has_parameter(self.prefix_ + "robot_constraints.mass"):
                descriptor = ParameterDescriptor(
                    description="Mass of the robot (kg)", read_only=True
                )
                descriptor.floating_point_range.append(FloatingPointRange())
                descriptor.floating_point_range[-1].from_value = 1.0
                descriptor.floating_point_range[-1].to_value = 100.0
                ...
                self.node_.declare_parameter(
                    self.prefix_ + "robot_constraints.mass", parameter, descriptor
                )

            if not self.node_.has_parameter(self.prefix_ + "robot_constraints.t_react"):
                descriptor = ParameterDescriptor(
                    description="Reaction time of the robot (seconds)", read_only=True
                )
                descriptor.floating_point_range.append(FloatingPointRange())
                descriptor.floating_point_range[-1].from_value = 0.1
                descriptor.floating_point_range[-1].to_value = 2.0
                ...
                self.node_.declare_parameter(
                    self.prefix_ + "robot_constraints.t_react", parameter, descriptor
                )
            ...
            if validation_result:
                raise InvalidParameterValueException(
                    "robot_constraints.v_max",
                    param.value,
                    "Invalid value set during initialization for parameter robot_constraints.v_max: "
                    + validation_result,
                )
            updated_params.robot_constraints.v_max = param.value
            ...
            if validation_result:
                raise InvalidParameterValueException(
                    "robot_constraints.omega_max",
                    param.value,
                    "Invalid value set during initialization for parameter robot_constraints.omega_max: "
                    + validation_result,
                )
            updated_params.robot_constraints.omega_max = param.value
            ...
            if validation_result:
                raise InvalidParameterValueException(
                    "robot_constraints.a_t_max",
                    param.value,
                    "Invalid value set during initialization for parameter robot_constraints.a_t_max: "
                    + validation_result,
                )
            updated_params.robot_constraints.a_t_max = param.value
            ...
            if validation_result:
                raise InvalidParameterValueException(
                    "robot_constraints.a_r_max",
                    param.value,
                    "Invalid value set during initialization for parameter robot_constraints.a_r_max: "
                    + validation_result,
                )
            updated_params.robot_constraints.a_r_max = param.value
            ...
            if validation_result:
                raise InvalidParameterValueException(
                    "robot_constraints.f_max",
                    param.value,
                    "Invalid value set during initialization for parameter robot_constraints.f_max: "
                    + validation_result,
                )
            updated_params.robot_constraints.f_max = param.value
            ...
            if validation_result:
                raise InvalidParameterValueException(
                    "robot_constraints.mass",
                    param.value,
                    "Invalid value set during initialization for parameter robot_constraints.mass: "
                    + validation_result,
                )
            updated_params.robot_constraints.mass = param.value
            ...
            if validation_result:
                raise InvalidParameterValueException(
                    "robot_constraints.t_react",
                    param.value,
                    "Invalid value set during initialization for parameter robot_constraints.t_react: "
                    + validation_result,
                )
            updated_params.robot_constraints.t_react = param.value
