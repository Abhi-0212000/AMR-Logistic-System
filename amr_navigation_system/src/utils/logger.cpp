...
: package_name_(package_name), node_name_(node_name), file_type_(file_type), log_level_(log_level)
{
  ...
  if (std::filesystem::exists(log_dir)) {
    ...
    if (std::filesystem::create_directories(log_dir)) {
      ...
      std::cerr << "Failed to create directories: " << strerror(errno) << " " << log_dir
                << std::endl;
    }
  }
...
NodeLogger::~NodeLogger()
{
  if (logger_) {
    ...
    RCLCPP_INFO(
      rclcpp::get_logger("NodeLogger"),
      "Log Messages are flushed to the log file. shutting down the spdlog.");
...
std::string NodeLogger::getCurrentTimeString()
{
  ...
  oss << std::put_time(std::gmtime(&now_time_t), "%Y-%m-%d_%H-%M-%S")  // Use gmtime for UTC
      << '.' << std::setfill('0') << std::setw(3) << now_ms.count() << " UTC";  // Append time zone
  return oss.str();
}
