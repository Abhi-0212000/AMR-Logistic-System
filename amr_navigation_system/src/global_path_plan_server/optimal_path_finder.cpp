  ...
  bool end_is_lanelet)
{
  ...
  auto tryPath =
    [&](const lanelet::ConstLaneletOrArea & start, const lanelet::ConstLaneletOrArea & end) {
      ...
      if (path) {
        ...
        logger_->log_debug(
          fmt::format("Trying path with IDs: {}.", path_ids.str().c_str()), __FILE__, __LINE__,
          __FUNCTION__);
        ...
        logger_->log_debug(
          fmt::format("Path distance: {} meters.", distance), __FILE__, __LINE__, __FUNCTION__);

        if (distance < shortestDistance) {
          bestPath = path;
          shortestDistance = distance;
          logger_->log_debug(
            fmt::format("New shortest path found with distance: {} meters.", shortestDistance),
            __FILE__, __LINE__, __FUNCTION__);
        }
      } else {
        logger_->log_warn(
          fmt::format("No valid path found from start ID {} to end ID {}.", start.id(), end.id()),
          __FILE__, __LINE__, __FUNCTION__);
      }
    };

  logger_->log_debug(
    fmt::format("Finding optimal path from start ID {} to end ID {}...", start_id, end_id),
    __FILE__, __LINE__, __FUNCTION__);
  ...
  logger_->log_info(
    fmt::format("Optimal path found with shortest distance: {} meters.", shortestDistance),
    __FILE__, __LINE__, __FUNCTION__);
  return bestPath.get();
}
  ...
  const lanelet::traffic_rules::TrafficRules & amrTrafficRules)
{
  ...
  for (const auto & element : path) {
    if (element.isLanelet()) {
      ...
      logger_->log_debug(
        fmt::format(
          "Lanelet ID {} added to path, Inverted: {}", lanelet.id(),
          lanelet.inverted() ? "true" : "false"),
        __FILE__, __LINE__, __FUNCTION__);
      ...
      logger_->log_debug(
        fmt::format("Area ID {} added to path.", area.id()), __FILE__, __LINE__, __FUNCTION__);
    }
  }
  ...
  double max_dist_threshold)
{
  logger_->log_debug(
    fmt::format(
      "Getting nearest lanelet or area for GPS point ({:.6f}, {:.6f}).", gps_point.latitude,
      gps_point.longitude),
    __FILE__, __LINE__, __FUNCTION__);
  ...
  //                  "No nearby navigable lanelet or area found.");
  ...
  if (min_distance > max_dist_threshold) {
    logger_->log_error(
      "No nearby navigable lanelet or area found within the threshold distance.", __FILE__,
  ...
  logger_->log_debug(
    fmt::format(
      "Nearest {} found: ID = {} at distance of {:.2f}",
      nearest_element.isLanelet ? "lanelet" : "area", nearest_element.id, min_distance),
    __FILE__, __LINE__, __FUNCTION__);

  return nearest_element;
}

lanelet::BasicPoint2d OptimalPathFinder::convertGPSToUTM(
  const GPSPoint & gps_point, const GPSPoint & origin_point)
{
  lanelet::projection::UtmProjector projector(
    lanelet::Origin({origin_point.latitude, origin_point.longitude}));
  ...
  logger_->log_debug(
    fmt::format(
      "Converted GPS ({:.6f}, {:.6f}) to UTM ({:.2f}, {:.2f}).", gps.lat, gps.lon, utm_point.x(),
      utm_point.y()),
    __FILE__, __LINE__, __FUNCTION__);
...
double OptimalPathFinder::calculatePathDistance(const lanelet::routing::LaneletOrAreaPath & path)
{
  ...
  logger_->log_debug(
    fmt::format("Total path distance: {:.2f} meters.", total_distance), __FILE__, __LINE__,
    __FUNCTION__);
...
double OptimalPathFinder::getLength(const lanelet::ConstLaneletOrArea & la) const
{
  ...
  logger_->log_debug(
    fmt::format("Length of lanelet/area (ID {}): {:.2f} meters.", la.id(), length), __FILE__,
    __LINE__, __FUNCTION__);

  return length;
}

double OptimalPathFinder::estimateTravelTime(
  const lanelet::routing::LaneletOrAreaPath & path,
  const lanelet::traffic_rules::TrafficRules & amrTrafficRules)
{
  ...
  logger_->log_debug(
    fmt::format("Total estimated travel time: {:.2f} seconds.", total_time), __FILE__, __LINE__,
    __FUNCTION__);

  return total_time;
}

double OptimalPathFinder::getTravelTime(
  ...
  const lanelet::traffic_rules::TrafficRules & amrTrafficRules) const
{
  ...
  if (la.isLanelet()) {
    ...
    if (laneletOpt) {
      ...
      logger_->log_debug(
        fmt::format(
          "Lanelet ID {}: Length = {:.2f} meters, Speed Limit = {:.2f} m/s, Estimated time = "
          "{:.2f} seconds.",
    ...
    if (areaOpt) {
      double perimeter =
        boost::geometry::perimeter(lanelet::utils::to2D(areaOpt.get().outerBoundPolygon()));
      lanelet::Velocity speedLimit = amrTrafficRules.speedLimit(areaOpt.get()).speedLimit;
      time = perimeter / speedLimit.value();
      logger_->log_debug(
        fmt::format(
          "Area ID {}: Perimeter = {:.2f} meters, Speed Limit = {:.2f} m/s, Estimated time = "
          "{:.2f} seconds.",
