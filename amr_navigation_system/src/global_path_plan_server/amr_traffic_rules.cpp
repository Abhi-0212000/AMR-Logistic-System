...
{
  return area.hasAttribute("amr_navigable") && area.attribute("amr_navigable").value() == "yes";
}
...
bool AmrTrafficRules::canPass(
  const lanelet::ConstLanelet & from, const lanelet::ConstArea & to) const
{
  return canPass(from) && canPass(to);
}
...
bool AmrTrafficRules::canPass(
  const lanelet::ConstArea & from, const lanelet::ConstLanelet & to) const
{
  return canPass(from) && canPass(to);
}
...
bool AmrTrafficRules::isOneWay(const lanelet::ConstLanelet & lanelet) const
{
  if (lanelet.hasAttribute("one_way") && lanelet.attribute("one_way").value() == "yes") {
    return !lanelet.hasAttribute("bidirectional") ||
           lanelet.attribute("bidirectional").value() != "yes";
  }
  return false;
}
